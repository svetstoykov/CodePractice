...
/// </para>
/// </remarks>
internal sealed class ImprovedUnicodeTokenCounter
{
    ...
    private static int CountJsonElement(JsonElement element, int parentObjectPropertyCount, int parentArrayItemCount) =>
        element.ValueKind switch
        {
            ...
            JsonValueKind.String => CountJsonValueString(element.GetString() ?? string.Empty, parentObjectPropertyCount, parentArrayItemCount),
    ...
    private static int CountJsonObject(JsonElement element)
    {
        ...
        return total + (propertyCount <= 3 ? propertyCount : CeilingDiv(propertyCount, 2));
    }
    ...
    private static int CountJsonArray(JsonElement element)
    {
        ...
        if (allCompactPrimitives)
        {
            total += CeilingDiv(itemCount, 3);
        }
        else if (itemCount > 1)
        {
    ...
    private static int CountJsonValueString(string text, int parentObjectPropertyCount, int parentArrayItemCount)
    {
        if (parentArrayItemCount > 0)
        {
            return parentArrayItemCount >= 4
                ? CountJsonString(text)
                : Math.Max(1, CountJsonStringContent(text));
        }

        if (parentObjectPropertyCount >= 4)
        {
